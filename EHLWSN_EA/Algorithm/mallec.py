class BatterySlot:
    ...
    def __repr__(self):
        return "<From %d to %d, %s, min=%.2f, max=%.2f, delta_full=%.2f delta_sleep=%.2f>" % (
            self.start, self.start + self.size, self.type, self.min, self.max, self.delta_e_full, self.delta_e_sleep)
...
def process_slots(slots, max_err, err_type, next_min_list, batt_delta, emin, emax):
    ...
    while idx < len(slots) and max_err > 0:
        ...
        if slots[idx].type == err_type and max(slots[idx].wasted, slots[idx].missed) > abs(batt_change):
            ...
            print("Error in slot", idx, max(slots[idx].wasted,
                                            slots[idx].missed), "couldn't be recovered. Change so far", batt_change)
            return None
        idx += 1
    if max_err > 0:
    ...
    for slot in e_in:
        ...
        if crt != prev and prev != None:
            ...
            if (batt_max - batt_min) > (b_max - b_min):
                ...
                # return None
            batt_slots.append(
                BatterySlot(prev, batt_slot_start, batt_slot_size, float(batt_sum) / batt_slot_size, batt_min, batt_max,
                            batt_wasted, batt_missed, batt_slot_size * e_max - total_e_cons_in_slot,
                            total_e_cons_in_slot - batt_slot_size * e_min))
...
def second_pass(batt_slots, bmin, bmax, emin, emax):
    ...
    while index < len(batt_slots):
        ...
        if batt_slots[index].error(batt_delta + tent_err, bmin, bmax) > 1e-6:
            ...
            if crt_err_type == None:
                ...
                next_min_list = next_minimum_list(
                    delta_list(batt_slots[list_start:max_err_index + 1], crt_err_type, bmax, bmin))
                # process list [list_start:max_err_index+1]
                process_rez = process_slots(batt_slots[list_start: max_err_index + 1], max_err, crt_err_type,
                                            next_min_list, batt_delta, emin, emax)
                if process_rez == None:
    ...
    if list_start < len(batt_slots) and max_err != 0:
        ...
        process_rez = process_slots(batt_slots[list_start:], max_err, crt_err_type, next_min_list, batt_delta, emin,
                                    emax)
        if process_rez == None:
            print("Second pass: couldn't recover error")
            return None
...
class MallecOptimal():
    def __init__(self, slots_per_cycle):
        self.allocation = [0 for i in range(slots_per_cycle)]
    ...
    def allocate(self, eh_pred, start_batt):
        if self.start_batt is None:
            self.start_batt = start_batt
        self.allocation, self.battery_pred, self.battery_slots = simple_optimum(self.start_batt, ehct.bmin, ehct.bmax,
                                                                                ehct.emin, ehct.emax, eh_pred)
        self.current_batt_slot = 0
        self.offset_in_batt_slot = 0
